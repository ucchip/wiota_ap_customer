 ...
 void manager_logic_send_to_operater(unsigned int request_number, unsigned int src_address, unsigned int cmd, unsigned int id, int len, void *data, manager_message_exe_result func)
{
    ...
    to_operation_wiota_data(MANAGER_OPERATION_MSG, page);
...
void manager_recv_data(unsigned int user_id, unsigned char *recv_data, unsigned int data_len, unsigned char type)
{
    ...
    //rt_kprintf("manager recv data from wiota\n");
...
static void manager_operate_result(uc_send_recv_t *res)
{
    ...
    else
    {
        ...
        else
        {
            ...
            // resend data
            uc_wiota_send_data(process->pload, process->len, &(process->head.id), 1, 10000, manager_send_result);
            process->head.state++;
            rt_sem_take(manager_list_sem, RT_WAITING_FOREVER);
    ...
    */
    ...
    while (0 != current_task_state)
    {
        ...
        case MANAGER_OPERATION_MSG:
        {
            ...
            {
                int num = 0;
                char *pload_data = data->pload;
                rt_kprintf("send data: ");
                for(num = 0; num < data->len; num++)
                    rt_kprintf("%x\t", pload_data[num]);
                rt_kprintf("\n");
            }
            #endif
            //send data to wiota
            uc_wiota_send_data(data->pload, data->len, &(data->head.id), 1, 10000, manager_send_result);
