...
static void *app_operation_handle = RT_NULL;
...
static void manager_start_replace_address_fun(void)
{
    // send MANAGER_LOGIC_START_REPLACE_ADDR  to "manager_wiota_task"
     to_queue_logic_data(MANAGER_LOGIC_INDENTIFICATION, MANAGER_LOGIC_START_REPLACE_ADDR, RT_NULL);
}
...
static void manager_send_replace_address_message(unsigned int old_address, unsigned int new_address)
{
    ...
    ps_header.addr.src_addr = AP_DEFAULT_ADDRESS; //ap address ??
    ...
    manager_logic_send_to_operater(ps_header.packet_num, ps_header.addr.src_addr, ps_header.cmd_type, old_address, output_len, output_data, manager_message_def_exe_result);
    rt_free(string_data);
    cJSON_Delete(root);
}
//unsigned int *test_memroy_address = RT_NULL;
...
void manager_wiota_task(void *pPara)
{
    ...
    */
    ...
    // init list
    //init_manager_list(&manager_wiota_list);
    ...
    while (1)
    {
        ...
        switch (message->cmd)
        {
            ...
            {
                manager_address_init(manager_start_replace_address_fun, manager_request_reserve_addr);
                //manager_get_reserved_address(0x0, 0x8);
                break;
            }
            ...
            case MANAGER_LOGIC_START_REPLACE_ADDR:
            {
                manager_operation_replace_address(manager_send_replace_address_message);
                break;
            }
    ...
    manager_thread_del(&app_operation_handle);
    app_operation_handle = RT_NULL;
}
