...
void *parse_mqtt_payload(cJSON *payload)
{
    if (RT_NULL == payload)
    {
        return RT_NULL;
        ...
        (dest_len = cJSON_GetArraySize(dest_addr_arr)))
    {
        ...
        if (RT_NULL != dest_address)
        {
            cJSON_ArrayForEach(dest_addr, dest_addr_arr)
            {
                ...
                {
                    dest_address[i] = (unsigned int)dest_addr->valueint;
                    i++;
                    if (i >= dest_len)
                    {
                        break;
    ...
    else if (cJSON_IsNumber(dest_addr_arr))
    {
        dest_len = 1;
        dest_address = (unsigned int *)rt_calloc(sizeof(unsigned int), dest_len);
        dest_address[0] = (unsigned int)dest_addr_arr->valueint;
    }
...
void passthrough_manager_task(void *pPara)
{
    ...
    while (1)
    {
        ...
        if (0 != manager_thread_create_task(&app_mqtt_wait_handle, "app_mqtt_wait_handle", uc_mqtt_wait_msg, RT_NULL, 256, 3, 5))
        {
        ...
        while (uc_mqtt_get_state() == STATE_CONNECT)
        {
            ...
            switch (page->cmd)
            {
                case APP_CMD_IOTE_PROPERTY_REPORT:
                {
                    ...
                    rt_memcpy(payload + rt_strlen(payload), "}", rt_strlen("}"));
                    uc_mqtt_publish(topic, (uint8_t*)payload, rt_strlen(payload), 0, 0, 0);
                ...
                case WIOTA_AP_REQUEST_RESERVE_ADDR:
                {
                    ...
                    dec2str(s_nRequestId++, topic);
                    cJSON *json_data = page->data;
                    cJSON *json_method_val = cJSON_CreateString("test");
                    cJSON_AddItemToObject(json_data, "method", json_method_val);
                    char *srtPayload = cJSON_Print(json_data);
                    if (NULL != srtPayload)
                    {
                        // push data
                        uc_mqtt_publish(topic, (uint8_t*)srtPayload, rt_strlen(srtPayload), 0, 0, 0);
                        free(srtPayload);
                    }
                    free(topic);
                    break;
                ...
                case MANAGER_LOGIC_MQTT_REQUEST:
                {
                    ...
                    dec2str(req->nId, topic);
                    char *res_ok = "{\"res\":\"ok\"}";
                    int nRet = uc_mqtt_publish(topic, (uint8_t*)res_ok, rt_strlen(res_ok), 0, 0, 0);
                    ...
                    // if (RT_NULL == payload)
                    // {
                    //     rt_kprintf("MANAGER_LOGIC_MQTT_REQUEST json parse error\n");
                    // }
                    // void *wiota_logic_info = parse_mqtt_payload(payload);
