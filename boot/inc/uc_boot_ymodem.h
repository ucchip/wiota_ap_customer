...
#ifndef __UC_BOOT_YMODEM_H__
#define __UC_BOOT_YMODEM_H__

/* The word "RYM" is stand for "Real-YModem". */

enum uc_boot_rym_code
{
    RYM_CODE_NONE = 0x00,
    RYM_CODE_SOH  = 0x01,
    RYM_CODE_STX  = 0x02,
...
/* RYM error code
 *
 * We use the int to return error values. We take use of current error
 ...
 */
typedef enum uc_boot_rym_code(*uc_boot_rym_callback)(struct uc_boot_rym_ctx* ctx, unsigned char * buf, unsigned int len);
...
struct uc_boot_rym_ctx
{
    uc_boot_rym_callback on_data;
    uc_boot_rym_callback on_begin;
    uc_boot_rym_callback on_end;
 ...
 * second.
 */
int uc_boot_rym_recv_on_device(uc_boot_rym_callback on_begin, uc_boot_rym_callback on_data, uc_boot_rym_callback on_end, int handshake_timeout);
 ...
 * second.
 */
int uc_boot_rym_send_on_device(uc_boot_rym_callback on_begin, uc_boot_rym_callback on_data, uc_boot_rym_callback on_end, int handshake_timeout);
