...
typedef struct{
volatile char *read;  /* 16 bit to avoid implicit padding (warnings) */
volatile char *write;
} PageType;
...
#define TRACE_SET_HIGH16(current, value) (((current) & 0x0000FFFF) | (((unsigned int )(value)) << 16))
...
#if 1 //(TRC_CFG_RECORDER_MODE == TRC_RECORDER_MODE_SNAPSHOT)
 ...
 ******************************************************************************/
//unsigned int  uiTraceGetTraceBufferSize(void);
 ...
 *
 ******************************************************************************/
void prvTracePortGetTimeStamp(unsigned int  *puiTimestamp);

void * prvTraceGetObjectHandle(traceObjectClass objectclass);
...
#define TRC_STREAM_PORT_ALLOCATE_EVENT(_type, _ptrData, _size)  _type* _ptrData = (_type*)prvPagedEventBufferGetWritePointer(_size);


#define TRC_STREAM_UPDATE_WRITE_PINT prvPagedEventBufferUpdate();
 ...
 * Example:
 * 
 * 	int32_t myRead(void* ptrData, unsigned int  size, int32_t* ptrBytesRead);
 * 
 ...
 *
 * - _size: the number of bytes to write (unsigned int ).
 *
 ...
 * Example:
 *
 * 	int32_t myWrite(void* ptrData, unsigned int  size, int32_t* ptrBytesWritten);
 *
...
//void prvTraceSaveObjectData(const void *address, unsigned int  data);
...
//void prvNewTraceStoreStringEvent(int nArgs, uint16_t eventID, const char* str, ...);
