...
static at_result_t at_wiota_sync_paging_setup(const char *args)
{
    ...
    uc_wiota_sync_paging(&paging_info, is_block ? RT_NULL : uc_wiota_paging_callback);
...
static at_result_t at_sync_paging_num_setup(const char *args)
{
    ...
    at_server_printfln("+WIOTASPNUM:%d", uc_wiota_get_sync_paging_num(group_idx, subf_idx));
...
static at_result_t at_wiota_sync_assistant_pps_setup(const char *args)
{
    ...
    uc_wiota_set_sync_assistant_pps(is_open);
...
static at_result_t at_wiota_iote_leaving_setup(const char *args)
{
    ...
    uc_wiota_iote_leaving_active_state(&user_id, 1);
...
static at_result_t at_wiota_ul_subf_mode_setup(const char *args)
{
    ...
    args = parse((char *)(++args), "d,y", &subf_mode, &user_id);
    if (!args)
    {
        return AT_RESULT_PARSE_FAILE;
    }

    uc_wiota_set_ul_subframe_mode(subf_mode, user_id);
...
static at_result_t at_send_dl_subf_data_setup(const char *args)
{
    ...
    if (data_len > 0)
    {
        ...
        uc_wiota_add_dl_subframe_data(subf_data, data_len, fn);
...
static at_result_t at_send_subf_mode_test_setup(const char *args)
{
    ...
    if (mode >= 0 && mode <= 2)
    {
        uc_wiota_set_subframe_test(mode);
    }
    else
    {
...
AT_CMD_EXPORT("AT+WIOTAULSUBF", "<is_open>,<user_id>", RT_NULL, RT_NULL, at_wiota_ul_subf_mode_setup, RT_NULL);
