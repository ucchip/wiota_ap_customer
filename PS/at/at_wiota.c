...
void send_data_callback(uc_send_recv_t *result)
{
    ...
    if (result_ == RT_EOK)
    {
        ...
        for (; node_p != &send_queue_s.object_list; node_p = node_p->next)
        {
            ...
            {
                at_server_printfln("+WIOTASENQUEUE:%d,0x%x,%d\n", result->result, result->user_id, tmp->identification);
...
void send_broadcast_data_callback(uc_result_e result)
{
    ...
    if (result_ == RT_EOK)
    {
        ...
        for (; node_p != &send_queue_s.object_list; node_p = node_p->next)
        {
            ...
            {
                at_server_printfln("+WIOTABCQUEUE:%d,%d,%d\n", result, tmp->identification, tmp->user_id == -1 ? 0 : 1);
                rt_list_remove(node_p);
                rt_free(tmp->data);
                rt_free(tmp);
                break;
