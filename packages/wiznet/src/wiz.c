...
static void wiz_print_net_info(void)
{
    wiz_NetInfo net_info = {0};
    ctlnetwork(CN_GET_NETINFO, (void*) &net_info);

    rt_kprintf("ETH MAC = %02x-%02x-%02x-%02x-%02x-%02x \r\n", net_info.mac[0], net_info.mac[1], net_info.mac[2], net_info.mac[3], net_info.mac[4], net_info.mac[5]);
    rt_kprintf("ETH IP  = %d.%d.%d.%d \r\n", net_info.ip[0], net_info.ip[1], net_info.ip[2], net_info.ip[3]);
    rt_kprintf("ETH GW  = %d.%d.%d.%d \r\n", net_info.gw[0], net_info.gw[1], net_info.gw[2], net_info.gw[3]);
    rt_kprintf("ETH SN  = %d.%d.%d.%d \r\n", net_info.sn[0], net_info.sn[1], net_info.sn[2], net_info.sn[3]);
    rt_kprintf("ETH DNS = %d.%d.%d.%d \r\n", net_info.dns[0], net_info.dns[1], net_info.dns[2], net_info.dns[3]);
    rt_kprintf("ETH ip is %s \r\n", net_info.dhcp == 1 ? "STATIC" : "DHCP");
}
...
static int wiz_netstr_to_array(const char *net_str, uint8_t *net_array)
{
    ...
    else
    {
        ...
        if (strstr(net_str, ":"))
        {
            ret = sscanf(net_str, "%x:%x:%x:%x:%x:%x", mac_addr + 0, mac_addr + 1, mac_addr + 2,
...
/* WIZnet set MAC address */
static int wiz_config_mac(char *mac, rt_uint8_t len)
{
    const unsigned int ALGO_MULTIPLIER = 0x15a4e35;
    const unsigned int ALGO_INCREMENT = 0x1;
    unsigned int rand_seed = 0;
    char mac_addr[18] = {0};

    if (RT_NULL == mac || len < 6)
        wiz_netstr_to_array(WIZ_DEFAULT_MAC, wiz_net_info.mac);
    else
    {
        if (mac[0] == 0 && mac[1] == 0 && mac[2] == 0 && mac[3] == 0 && mac[4] == 0 && mac[5] == 0)
        {
            /* Random number address: 0x1A107080 */
            rand_seed = *(uint32_t*)0x1A107080;
            ...
            for (int i = 3; i < 6; ++i)
            {
                rand_seed = rand_seed * ALGO_MULTIPLIER + ALGO_INCREMENT;
                mac[i] = (unsigned char)((rand_seed >> 16) & 0xFF);
            }
        }
        rt_snprintf(mac_addr, 18, "%02x-%02x-%02x-%02x-%02x-%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
        if (wiz_netstr_to_array(mac_addr, wiz_net_info.mac) != RT_EOK)
        {
            rt_kprintf("invalid MAC address %s\n", mac_addr);
            return -RT_ERROR;
        }
    }
    return RT_EOK;
...
/* WIZnet set IP address */
static int wiz_config_ip(char *ip, rt_uint8_t len)
{
    if (RT_NULL == ip || len < 8)
    {
    #ifdef WIZ_IPADDR
    ...
    {
        if (wiz_netstr_to_array(ip, wiz_net_info.ip) != RT_EOK)
        {
            rt_kprintf("invalid IP address %s\n", ip);
            return -RT_ERROR;
        }
...
/* WIZnet set Mask address */
static int wiz_config_mask(char *mask, rt_uint8_t len)
{
    if (RT_NULL == mask || len < 8)
    {
    #ifdef WIZ_MSKADDR
        if (wiz_netstr_to_array(WIZ_MSKADDR, wiz_net_info.sn) != RT_EOK)
        {
            rt_kprintf("invalid mask address %s\n", WIZ_MSKADDR);
            return -RT_ERROR;
        }
    #else
        return -RT_ERROR;
    ...
    {
        if (wiz_netstr_to_array(mask, wiz_net_info.sn) != RT_EOK)
        {
            rt_kprintf("invalid mask address %s\n", mask);
            return -RT_ERROR;
        }
...
/* WIZnet set gateway address */
static int wiz_config_gateway(char *gateway, rt_uint8_t len)
{
    if (RT_NULL == gateway || len < 8)
    {
    #ifdef WIZ_GWADDR
        if (wiz_netstr_to_array(WIZ_GWADDR, wiz_net_info.dns) != RT_EOK ||
            wiz_netstr_to_array(WIZ_GWADDR, wiz_net_info.gw) != RT_EOK)
        {
            rt_kprintf("invalid gateway address %s\n", WIZ_GWADDR);
            return -RT_ERROR;
        }
    #else
        return -RT_ERROR;
    ...
    {
        if (wiz_netstr_to_array(gateway, wiz_net_info.dns) != RT_EOK ||
            wiz_netstr_to_array(gateway, wiz_net_info.gw) != RT_EOK)
        {
            rt_kprintf("invalid gateway address %s\n", gateway);
            return -RT_ERROR;
        }
...
{
    wiz_config_mac(mac, 6);
    /* print MAC address */
    LOG_I("MAC %02x-%02x-%02x-%02x-%02x-%02x",
        wiz_net_info.mac[0], wiz_net_info.mac[1], wiz_net_info.mac[2],
        wiz_net_info.mac[3], wiz_net_info.mac[4], wiz_net_info.mac[5]);
    /* static IP config */
    wiz_net_info.dhcp = static_ip == 0 ? NETINFO_DHCP : NETINFO_STATIC;
    if (wiz_net_info.dhcp == NETINFO_STATIC)
    {
        if (wiz_config_ip(ip, 16) != RT_EOK ||
            wiz_config_mask(mask, 16) != RT_EOK ||
            wiz_config_gateway(gateway, 16) != RT_EOK)
        {
            return -RT_ERROR;
        }
        ...
        if (wiz_net_info.dhcp == NETINFO_STATIC)
        {
            LOG_I("IP %d.%d.%d.%d", wiz_net_info.ip[0], wiz_net_info.ip[1], wiz_net_info.ip[2], wiz_net_info.ip[3]);
            LOG_I("MSK %d.%d.%d.%d", wiz_net_info.sn[0], wiz_net_info.sn[1], wiz_net_info.sn[2], wiz_net_info.sn[3]);
            LOG_I("GW %d.%d.%d.%d", wiz_net_info.gw[0], wiz_net_info.gw[1], wiz_net_info.gw[2], wiz_net_info.gw[3]);
        }
    }
    return RT_EOK;
}
...
#ifdef WIZ_USING_DHCP
static unsigned char wiz_ip_state = 0;
...
static int wiz_network_init(rt_bool_t b_config)
{
    ...
    if (wiz_net_info.dhcp == NETINFO_DHCP)
    {
...
int wiz_init(void)
{
    ...
    rt_pin_mode(WIZ_RST_PIN, PIN_MODE_OUTPUT_OD);
    ...
    tid = rt_thread_create("wiz_stat", wiz_link_status_thread_entry, RT_NULL, 1024, RT_THREAD_PRIORITY_MAX - 4, 20);
    if (tid != RT_NULL)
    {
        rt_thread_startup(tid);
    }
