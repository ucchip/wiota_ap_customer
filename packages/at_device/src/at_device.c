...
{
    LOG_I("enter at_device_class_register()\r\n");
    rt_base_t level;

    RT_ASSERT(class);
    LOG_I("at_device_class_register RT_ASSERT not error\r\n");

    /* Fill AT device class */
    class->class_id = class_id;

    /* Initialize current AT device class single list */
    LOG_I("Initialize current AT device class single list\r\n");
    rt_slist_init(&(class->list));
    LOG_I("ready rt_hw_interrupt_disable()\r\n");
    level = rt_hw_interrupt_disable();
    LOG_I("ready Add current AT device class to list\r\n");
    /* Add current AT device class to list */
    rt_slist_append(&at_device_class_list, &(class->list));
    LOG_I("ready rt_hw_interrupt_enable()\r\n");
    rt_hw_interrupt_enable(level);
    LOG_I("leave at_device_class_register()\r\n");
    return RT_EOK;
}
...
{
    LOG_D("enter at_device_register()\r\n");
    rt_base_t level;
    int result = 0;
...
__exit:
    if (result < 0)
    {
        ...
        LOG_D("dev ops init FALSE\r\n");
    }
    else
    {
        device->is_init = RT_TRUE;
        LOG_D("dev ops init TRUE\r\n");
    }
