...
void uc_uart_init(UART_TYPE *uartx, uint32_t baud_rate, uint8_t data_bits, uint8_t stop_bits, uint8_t parity)
{
    ...
    *(volatile unsigned int*)((uint32_t)uartx - UART_BASE_ADDR + UART_REG_FCR) = 0xA7; //enables 16byte FIFO and clear FIFOs
    *(volatile unsigned int*)((uint32_t)uartx - UART_BASE_ADDR + UART_REG_LCR) = line_reg; //sets 8N1 and set DLAB to 0
...
char uc_uart_getchar(UART_TYPE *uartx, uint8_t *get_char)
{
    ...
    //return *(volatile int*)UART_REG_RBR;
    if ((*((volatile int*)((uint32_t)uartx - UART_BASE_ADDR + UART_REG_LSR)) & 0x1) == 0x1)
    {
        //return *(volatile int*)((uint32_t)uartx - UART_BASE_ADDR + UART_REG_RBR);
        *get_char = *(volatile int*)((uint32_t)uartx - UART_BASE_ADDR + UART_REG_RBR);
        return 0;
    }
    else
    {
