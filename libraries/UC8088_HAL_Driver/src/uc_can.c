...
void CAN_Setup(void)
{
    // set_gpio_pin_direction(UC_GPIO, GPIO_PIN_10, GPIO_DIR_IN);
    set_gpio_pin_direction(GPIO_PIN_10, GPIO_DIR_IN);
    set_gpio_pin_mux(UC_GPIO_CFG, GPIO_PIN_10, GPIO_FUNC_2);

    // set_gpio_pin_direction(UC_GPIO, GPIO_PIN_11, GPIO_DIR_OUT);
    set_gpio_pin_direction(GPIO_PIN_11, GPIO_DIR_OUT);
    set_gpio_pin_mux(UC_GPIO_CFG, GPIO_PIN_11, GPIO_FUNC_2);

    // set_gpio_pin_value(UC_GPIO, GPIO_PIN_15, GPIO_VALUE_LOW);
    set_gpio_pin_value(GPIO_PIN_15, GPIO_VALUE_LOW);
}
...
uint8_t CAN_Transmit(CAN_CTRL_TypeDef *can_ctrl, CAN_TXMailBox_TypeDef *CAN_Tx, CanTxMsg *TxMessage)
{
    ...
    CAN_Tx->TDLR = (((uint32_t)TxMessage->Data[3] << 24) |
                    ((uint32_t)TxMessage->Data[2] << 16) |
                    ((uint32_t)TxMessage->Data[1] << 8) |
                    ((uint32_t)TxMessage->Data[0]));
    CAN_Tx->TDHR = (((uint32_t)TxMessage->Data[7] << 24) |
