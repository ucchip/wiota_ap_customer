...
#define WAIT_FOR_WR_DONE while (Flash_Read_SR(FLASH_CMD_READ_STATUS1) & 0x01)
...
__critical uint32_t Flash_Read_SR(uint8_t cmd)
{
    uint32_t data;
    REG_SPI_CMD = cmd << 24; //read sr
...
__critical void Flash_Write_SR(uint8_t cmd, uint8_t value)
{
    WAIT_FOR_WR_DONE;
    WAIT_XIP_FREE;
    FlashEnableWr();
    flash_delay(0);
    REG_SPI_CMD = cmd << 24 | value << 16; // set cmd
...
__critical void FlashQRead(uint32_t nAddr, uint8_t *pData, uint16_t usLen)
{
    spi_read_fifo(NULL, 0);
    WAIT_XIP_FREE;
    REG_SPI_CMD = FLASH_CMD_FREAD << 24; // set cmd
    REG_SPI_ADDR = (nAddr << 8);
    REG_SPI_LEN = 0x1808 | (usLen << 19); // set cmd,addr and data len
    WAIT_XIP_FREE;
    SPI_START(SPI_CMD_RD);
...
__critical void FlashProgramSecurity(uint32_t nAddr, const uint8_t *pData, uint16_t usLen)
{
    ...
    REG_SPI_CMD = FLASH_CMD_PROGRAM_SEC << 24;
    REG_SPI_ADDR = (nAddr << 8);
    /* spi len reg format *
     * bit16: bit15:8  bit7:0
     * DLEN   ADDRLEN  CMDLEN
...
__critical void FlashReadSecOld(uint32_t nAddr, uint8_t *pData, uint16_t usLen)
{
    //WAIT_FOR_WR_DONE;
    spi_read_fifo(NULL, 0);
    WAIT_XIP_FREE;
